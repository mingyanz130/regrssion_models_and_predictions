...
cor(climate$temp,climate$n2o)
cor(climate$temp,climate$hcfc)
cor(climate$temp,climate$sf6)


#cor(nasatemp,maunaloa1$co2)
#plot(nasatemp,maunaloa1$co2)



qplot(co2, temp, data = climate,
      geom = "point",
     ylab = "Atmosphere Carbon Dioxide (ppm)",
     xlab= "Global Surface Temperature Anamoly(0.01 degrees C)")

qplot(methane, temp,data = climate,
      geom = "point",
     ylab = "Atmosphere methane (ppm)",
     xlab= "Global Surface Temperature Anamoly(0.01 degrees C)")

qplot(n2o, temp, data = climate,
      geom = "point",
      ylab = "Atmosphere n2o (ppm)",
      xlab= "Global Surface Temperature Anamoly(0.01 degrees C)")

qplot(hcfc, temp,data = climate,
      geom = "point",
      ylab = "Atmosphere hcfc(ppm)",
      xlab= "Global Surface Temperature Anamoly(0.01 degrees C)")

qplot(sf6, temp,data = climate,
      geom = "point",
      ylab = "Atmosphere sf6(ppm)",
      xlab= "Global Surface Temperature Anamoly(0.01 degrees C)")

#Analysis

#Response Varaibles: temp
#Explanatory Varaible: 
#     Carbon Dioxide(co2,ppm)
#     Methane (CH4, ppm)

#Model: temp = beta0 + beta1*co2 + beta2* methane+ bea3*n2o + beta4* hcfc+ beta4*sf6 + epsilon, epsilon~N(0,sigma2)

climate.out = lm(temp ~ co2 + methane + n2o + hcfc + sf6, data = climate)

#table of estimate & std Errors
summary(climate.out)

##the summary dose not make sense, co2 has a negaive esimate

#to see he collinearity, plot explanatory
plot(~co2+methane+n2o+hcfc+sf6, data = climate)

#because of collinaerity let's fi a subse regression model
climate.out1 = lm(temp ~ co2+ methane+sf6, data = climate)
summary(climate.out1)

##all numbers make sense

#if the inference is the goal
climate.out2 = lm(temp ~ co2+ methane, data = climate)
summary(climate.out2)




#plots showing the effect of co2 and ch4

library(car)
#component plus residual(temp) plot
crPlots(climate.out)
...
#h0 test: no greenhouse gas effect
noeffect <- lm(temp ~ +1, data=climate)
anova(noeffect, climate.out)


#to identify which gases are significant
#t-test
summary(climate.out)
...
#Access prediction performance
climate.train <- subset(climate, year <= 2016)
climate.test <- subset(climate, year == 2017)

climate.train.out <- lm(temp~co2+methane, data = climate.train)

#table of last year's prediction and actual
cbind(climate.test$temp, predict(climate.train.out, newdata=climate.test))

#graphic
plot(predict(climate.train.out, newdata=climate.test),climate.test$temp) 
abline(0,1,col="gray")
