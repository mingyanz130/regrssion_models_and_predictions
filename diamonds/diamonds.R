...
#F = 6875.1
#pvalue = < 0.00001
# we reject H0 in favor of Ha: beta1 != 0 since pvalue< 0.00001 < alpha = 0.05
# There is a statically significant inheritance effect (pvalue = 0.0029)
#For a one unit increase in log(Carat) we estimate a 1.537 increase in log(Price) on average 


#95% confidence Interval on beta1
confint(diamonds.out)
#(1.501 1.574)
#  we reject H0 in favor of Ha: beta1 != 0 since the 95% confidence interval on beta1 is (1.501 1.574) which 
#dose not contain zero
# There is a statically significant inheritance effect (pvalue < 0.00001)
#For a one unit increase in log(Carat) we estimate a 1.537 (95%CI: 1.501 1.574) increase 
#in log(Price) on average 
...
exp(predict(diamonds.out,newdata=data.frame(Carat=1),interval="confidence"))


# summary statistics of absolute prediction error

# remember to un-transform!
